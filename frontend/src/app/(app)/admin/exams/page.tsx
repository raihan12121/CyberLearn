...
export default function AdminExamsPage() {
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-8 pb-16">
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <div className="flex items-center gap-2.5">
              <h1 className="text-xl sm:text-2xl font-bold text-white tracking-tight">Certification Exams & Question Bank</h1>
          ...
          >
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 line-clamp-2 mt-1">{ex.description}</p>
              </div>

              <div className="flex items-center gap-3 text-xs text-slate-400 pt-2 border-t border-[#1E293B]">
                <span className="flex items-center gap-1">
                  <Clock className="w-3.5 h-3.5 text-blue-400" /> {ex.duration_minutes} mins
                </span>
                <span>•</span>
                ...
                <span>•</span>
                <span className="text-slate-300 font-mono">{ex.total_marks} Marks</span>
              </div>
            </div>
      ...
      <Card padding="none" className="border border-[#1E293B] bg-[#0F172A] shadow-lg overflow-hidden space-y-0">
        <div className="p-4 border-b border-[#1E293B] bg-[#0C1222] flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-white flex items-center gap-2">
              <Award className="w-4 h-4 text-blue-400" /> Student Certification Attempts & Submissions Ledger
            </h3>
            <p className="text-xs text-slate-400 mt-0.5">
                ...
                submissionsList.map((sub) => (
                  <tr key={sub.id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="py-3 px-4">
                      <p className="font-bold text-white">{sub.user_name}</p>
                      <p className="text-[10px] text-slate-400 font-mono">{sub.user_email}</p>
                    </td>
                    <td className="py-3 px-4 font-semibold text-slate-200">{sub.exam_title}</td>
                    <td className="py-3 px-4 font-mono font-bold text-slate-300">{sub.score} Marks</td>
                    <td className="py-3 px-4 font-mono font-bold text-blue-400">{sub.score_pct}%</td>
                    <td className="py-3 px-4">
                      {sub.passed ? (
          ...
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between border-b border-[#1E293B] pb-3">
                <h3 className="text-base font-bold text-white">Create Certification Exam</h3>
              ...
              <form onSubmit={handleCreateExam} className="space-y-3.5 text-xs">
                <div className="space-y-1">
                  <label className="font-semibold text-white">Exam Slug / ID *</label>
                  <input
                    required
                    type="text"
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-white">Exam Title *</label>
                  <input
                    required
                    type="text"
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="space-y-1">
                    <label className="font-semibold text-white">Passing Score (%)</label>
                    <input
                      type="number"
                  ...
                  <div className="space-y-1">
                    <label className="font-semibold text-white">Total Marks</label>
                    <input
                      type="number"
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-white">Instructions & Syllabus Scope</label>
                  <textarea
                    rows={3}
      ...
      <AnimatePresence>
        {editingExam && (
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between border-b border-[#1E293B] pb-3">
                <h3 className="text-base font-bold text-white">Edit Exam: {editingExam.title}</h3>
                <button onClick={() => setEditingExam(null)} className="text-slate-400 hover:text-white">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form onSubmit={handleUpdateExam} className="space-y-3.5 text-xs">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="space-y-1">
                    <label className="font-semibold text-white">Pass (%)</label>
                    <input
                      type="number"
                  ...
                  <div className="space-y-1">
                    <label className="font-semibold text-white">Marks</label>
                    <input
                      type="number"
            ...
            >
              <div className="flex items-center justify-between border-b border-[#1E293B] pb-3">
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-mono">{activeExamQuestions.exam.id}</p>
                </div>
                <button onClick={() => setActiveExamQuestions(null)} className="text-slate-400 hover:text-white">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-3 pr-1 scrollbar-thin">
                {activeExamQuestions.questions.length === 0 ? (
                  ...
                  activeExamQuestions.questions.map((q, idx) => (
                    <div
                      key={q.id}
                      className="p-4 rounded-xl bg-[#0C1222] border border-[#1E293B] space-y-2 hover:border-slate-700 transition-colors"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-start gap-2.5">
                          <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-blue-500/10 text-blue-400 shrink-0">
                            Q{idx + 1}
                          </span>
                          <div>
                        ...
                        <Button
                          ...
                          className="hover:bg-red-500/10 text-red-400 shrink-0"
                          icon={<Trash2 className="w-3.5 h-3.5" />}
                        />
                      </div>

                      {/* Options breakdown */}
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 pt-2 border-t border-[#1E293B]/60 text-xs">
                        {q.options?.map((opt: string, optIdx: number) => (
                          ...
                          >
                            <span className="w-4 h-4 rounded-full bg-slate-800 text-[10px] flex items-center justify-center shrink-0">
                      ...
                      {q.explanation && (
                        <p className="text-[11px] text-slate-400 bg-slate-800/30 p-2 rounded-lg border border-slate-700/40">
      ...
      <AnimatePresence>
        {showAddQuestionModal && (
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between border-b border-[#1E293B] pb-3">
                <h3 className="text-base font-bold text-white">Add Exam Question</h3>
              ...
              <form onSubmit={handleCreateQuestion} className="space-y-3 text-xs">
                <div className="space-y-1">
                  <label className="font-semibold text-white">Question Prompt / Scenario *</label>
                  <textarea
                    required
                    rows={2}
                ...
                <div className="space-y-2">
                  <label className="font-semibold text-white">Answer Choices (MCQ)</label>
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      className="bg-[#0C1222] border border-[#1E293B] rounded-xl px-3 py-1.5 text-white focus:outline-none"
                    />
                    <input
                      required
                      type="text"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="font-semibold text-white">Correct Answer Index</label>
                    <select
                  ...
                  <div className="space-y-1">
                    <label className="font-semibold text-white">Points / Weight</label>
                    <input
                      type="number"
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-white">Explanation (Shown after exam)</label>
                  <input
                    type="text"
